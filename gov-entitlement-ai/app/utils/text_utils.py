...
def is_text_corrupted(elements):
    ...
    checked_count = min(total, 20)
    glyph_ratio = glyph_count / checked_count if checked_count > 0 else 0
    meaningful_ratio = meaningful_text / checked_count if checked_count > 0 else 0

    # If there are very few elements, only flag as weak extraction if there is zero meaningful text
    if total < 5:
        is_weak = (meaningful_text == 0)
    else:
        is_weak = (meaningful_ratio < 0.3)
