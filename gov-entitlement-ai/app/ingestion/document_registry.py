...
def register_document(file_path, metadata):
    ...
    try:
        ...
        if existing:
            ...
            print("\nDuplicate document detected - resuming/re-processing")
            print("----------------------------------")
            ...
            # Log duplicate detection but allow continuation for resumption
            log_ingestion_event({
                ...
                "stage": "REGISTERED",
                "status": "success",
                "message": "Duplicate document detected, returning existing ID for re-processing"
            })
        ...
        # ------------------------------------------------
        # STEP 2.5 — METADATA SANITIZATION & NORMALIZATION
        # ------------------------------------------------
        ...
        if doc_type:
            ...
            mapping = {
                ...
                "form": "rule", "forms": "rule", "application": "rule"
            }
            doc_type = mapping.get(doc_type, "report")
        else:
            doc_type = "report"
