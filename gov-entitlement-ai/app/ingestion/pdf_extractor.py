...
class PDFExtractor:
    ...
    def __init__(self):
        ...
        # Threading lock for Docling DocumentConverter (which is not thread-safe)
        import threading
    ...
    def split_pdf_pages(self, pdf_path):
        """
        Split PDF into single-page PDFs for safer Docling processing.
        """
    ...
    def extract_elements(self, pdf_path, lang=None, progress_callback=None):
        ...
        if scanned:
            ...
            pdf_path, ocr_stats = create_searchable_pdf(pdf_path, lang=lang)
        ...
        # 🔥 HARDWARE ACCELERATION (AUTO CPU/GPU)
        pipeline_options.accelerator_options = AcceleratorOptions(
            device=AcceleratorDevice.AUTO
        )
        
        logger.info(f"Using accelerator: {pipeline_options.accelerator_options.device}")
        ...
        try:
            ...
            total_steps = 2 * total_pages
            steps_completed = 0

            # 🔥 STEP 2: PROCESS LIGHT PAGES (PARALLEL)
            with ThreadPoolExecutor(max_workers=3) as executor:
                futures = {}

                for page_file in light_pages:
                    page_index = self.get_page_index(page_file)
                    if page_index is None:
                        continue

                    future = executor.submit(self._process_single_page_with_stats, page_file)
            ...
            # 🔥 STEP 3: PROCESS MEDIUM PAGES (LIMITED PARALLEL)
            with ThreadPoolExecutor(max_workers=2) as executor:
                futures = {}

                for page_file in medium_pages:
                    page_index = self.get_page_index(page_file)
                    if page_index is None:
                        continue
            ...
            # 🔥 STEP 4: PROCESS HEAVY PAGES (SEQUENTIAL — CRITICAL)
            for page_file in heavy_pages:
                page_index = self.get_page_index(page_file)
                ...
                try:
                    ...
                    logger.warning(f"Heavy page failed: {e}")
                    results_with_index.append((page_index, None))  
            ...
            for res in results:
                ...
                if is_corrupt:
                    ...
                    try:
                        ocr_pdf, ocr_stats = create_searchable_pdf(page_file, lang=lang)
                        ...
                        logger.warning(f"OCR reprocessing failed for page {page_file}: {ocr_err}", exc_info=True)
                        extraction_stats["failed_pages"] += 1
                        steps_completed += 1
                        if progress_callback:
                            try:
