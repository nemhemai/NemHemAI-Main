...
from app.agents.entitlement_context import FIELD_DOCUMENTS, check_edge_cases
from app.agents.entitlement_graph import match_entitlement_graph
from app.agents.entitlement_memory import remember_entitlement_result
...
def _evaluate_rule(
    ...
    # Standard logic: if ineligible, clear verification checklists
    if status == "NOT_ELIGIBLE":
        missing_documents = []
...
def build_determination(conn: Any, raw_query: str, profile: dict[str, Any], trigger_llm: bool = True) -> tuple[list[dict], dict]:
    ...
    for rule in rules:
        ...
        evaluation = _evaluate_rule(rule, profile, graph_match)
        verdict = evaluation["verdict"]
...
def run_entitlement_check(query_id: str, raw_query: str, user_id: str = None):
    conn = get_db_conn()
    try:
        with conn.cursor() as cur:
            cur.execute(
        ...
        scheme_matches, determination = build_determination(conn, raw_query, profile, trigger_llm=False)
