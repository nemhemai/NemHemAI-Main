...
EMBEDDING_MODEL = os.getenv("EMBED_MODEL_PATH", "BAAI/bge-m3")
# Resolve local pre-downloaded model weights if running inside workspace
if os.path.exists(os.path.join(os.getcwd(), "models/bge-m3")):
    EMBEDDING_MODEL = os.path.join(os.getcwd(), "models/bge-m3")
elif os.path.exists(r"d:\murtaza\AIML\NemHemAI-Main - Copy\NemHemAI-Main - Copy\models\bge-m3"):
    EMBEDDING_MODEL = r"d:\murtaza\AIML\NemHemAI-Main - Copy\NemHemAI-Main - Copy\models\bge-m3"

DENSE_DIM            = 1024             # BGE-M3 dense output dimension
DEFAULT_BATCH_SIZE   = 32               # chunks per encode call
