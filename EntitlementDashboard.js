...
function EntitlementDashboard() {
  ...
  const [filePath, setFilePath] = useState("C:/documents/income_proof.pdf");
  ...
  const loadAuditLogs = useCallback(async () => {
    ...
    try {
      const logs = await fetchAuditLogs(citizenId);
      setAuditLogs(logs);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleRegister = async () => {
    ...
    if (!validateVerhoeff(regAadhaar)) {
      setRegMessage("Aadhaar ID failed Verhoeff checksum validation.");
      setRegSuccess(false);
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        setName(profileRes.personal_info.name || "");
      } else {
        setRegMessage(res.message);
      }
    } catch (err) {
  ...
  const handleVerifyDocument = async () => {
    ...
    if (!filePath.trim()) {
      alert("Document File Path is required.");
      return;
    }
    setVerifyLoading(true);
    setVerificationPipelineLogs(null);
    try {
      const log = await verifyDocument({
        citizenId,
    ...
    <div style={styles.container}>
      ...
      <nav style={styles.tabNav}>
        {[
          ...
          { id: "guidance", label: "5. Guidance & Submissions" },
      ...
      <main style={styles.panelCard}>
        ...
        {activeTab === "verification" && (
          <div style={styles.panelContent}>
            ...
            <div style={styles.formGroup}>
              <label style={styles.formLabel}>Document File Path</label>
              <input
                type="text"
                value={filePath}
            ...
            <button
              ...
              style={styles.actionButton}
            >
              {verifyLoading ? "Executing Verification Pipelines..." : "Run Document Verification"}
            </button>

            {verificationPipelineLogs && (
              <div style={styles.pipelineLogBox}>
                <h3 style={styles.subHeading}>Verification Agent Step-by-Step Executed Log</h3>
                
                <div style={styles.stepLogItem}>
                  <strong>1. Document Classifier</strong>
                  <span>Detected type: <strong>{verificationPipelineLogs.document_classification.detected_type}</strong> (Confidence: {verificationPipelineLogs.document_classification.confidence * 100}%)</span>
                </div>
                
                <div style={styles.stepLogItem}>
                  <strong>2. OCR Extraction & 3. Field Parser</strong>
                  <span>Extracted fields: <em>{JSON.stringify(verificationPipelineLogs.ocr_extraction.extracted_fields)}</em> (Confidence: {verificationPipelineLogs.ocr_extraction.ocr_confidence * 100}%)</span>
                </div>

                <div style={styles.stepLogItem}>
                  <strong>4. Verification Rules</strong>
                  <span>Passed checks: {verificationPipelineLogs.rules_evaluation.rules_passed.join(", ") || "None"}</span>
                </div>

                <div style={styles.stepLogItem}>
                  <strong>5. Cross Validation</strong>
                  <span>Matched profile fields: {JSON.stringify(verificationPipelineLogs.cross_validation.matched_with_profile_fields)}</span>
                </div>

                <div style={styles.stepLogItem}>
                  <strong>6. Fraud Detection</strong>
                  <span>Is Tampered: <strong>{verificationPipelineLogs.fraud_detection.is_tampered ? "YES" : "NO"}</strong> | Risk Score: {verificationPipelineLogs.fraud_detection.risk_score}</span>
                  {verificationPipelineLogs.fraud_detection.anomaly_detected && (
                    <div style={styles.errorText}>Discrepancies: {verificationPipelineLogs.fraud_detection.anomaly_details}</div>
                  )}
                </div>

                <div style={{
                  ...styles.stepLogItem, 
                  borderLeft: `4px solid ${verificationPipelineLogs.verification_result.status === "VERIFIED" ? "#176b3a" : "#b42318"}`
                }}>
                  <strong>7. Verification Result</strong>
                  <span>Status: <strong style={{color: verificationPipelineLogs.verification_result.status === "VERIFIED" ? "#176b3a" : "#b42318"}}>{verificationPipelineLogs.verification_result.status}</strong> (Confidence: {verificationPipelineLogs.verification_result.overall_confidence * 100}%)</span>
                  <p style={styles.subtleText}>{verificationPipelineLogs.verification_result.reason}</p>
                </div>
                
                {/* Step 8: Reconfirm Eligibility */}
                <div style={styles.reconfirmActionBlock}>
                  <h4>Step 8: Eligibility Reconfirmation</h4>
                  <p style={styles.infoText}>Consumes verified documents and citizen profile details to make the final eligibility decision.</p>
                  <button onClick={handleReconfirm} style={styles.secondaryButton}>Reconfirm Eligibility Status</button>
                  
                  {reconfirmResult && (
                    <div style={styles.reconfirmResultCard}>
                      <div>Overall Decision Status: <strong style={{color: "#243c8f"}}>{reconfirmResult.overall_decision}</strong></div>
                      <ul>
                        {reconfirmResult.schemes?.map((s) => (
                          <li key={s.scheme_name}>
                            {s.scheme_name}: Preliminary verdict was <strong>{s.preliminary_verdict}</strong>, Document check: <strong>{s.verification_check}</strong>. Final Decision: <strong style={{color: s.final_decision === "ELIGIBLE" ? "#176b3a" : "#8a5a00"}}>{s.final_decision}</strong>
                          </li>
                        ))}
                      </ul>
                    </div>
        ...
        {/* Tab 5: Guidance & Submissions */}
        {activeTab === "guidance" && (
          <div style={styles.panelContent}>
            <h2 style={styles.sectionHeading}>Application Guidance & Submission Support</h2>
            <p style={styles.infoText}>
              Steps 11, 12 & 13: Help fill forms, verify checklist, generate legal affidavits, submit applications through API/Portal, and track live stages.
            </p>
            {!citizenId && <div style={styles.warningAlert}>A citizen must be registered first (Tab 1).</div>}
            
            <div style={styles.splitGrid}>
              <div>
                <h3 style={styles.subHeading}>Step 11: Application Assistance</h3>
                <div style={styles.formGroup}>
                  <label style={styles.formLabel}>Select Scheme</label>
                  <select
                  ...
                  >
                    ...
                    <option value="PMAY-U">PMAY-U</option>
                ...
                <button
                  ...
                  disabled={guidanceLoading || !citizenId}
                  style={styles.secondaryButton}
                >
                  {guidanceLoading ? "Generating Guidance..." : "Load Form Pre-fill & Guidance"}
                </button>

                {guidanceData && (
                  <div style={styles.guidanceDataCard}>
                    <h4>Prefilled Form Values:</h4>
                    <pre style={styles.preformatted}>{JSON.stringify(guidanceData.prefilled_form, null, 2)}</pre>
                    
                    <h4>Required Legal Self-Declaration / Affidavit Text:</h4>
                    <div style={styles.affidavitBox}>
                      {guidanceData.affidavit_text}
                    </div>
                    
                    {guidanceData.missing_documents.length > 0 ? (
                      <div style={styles.warningText}>
                        ⚠️ Missing documents: <strong>{guidanceData.missing_documents.join(", ")}</strong>. Please verify documents in Tab 4 first.
                      </div>
                    ) : (
                    ...
                    <div style={styles.submissionBlock}>
                      <h4>Step 12: submission Support</h4>
                      <p style={styles.infoText}>Submit to official databases through the following channels:</p>
                      <div style={styles.buttonGroup}>
                        <button onClick={() => handleAppSubmit("API")} disabled={submittingApp} style={styles.channelButton}>Submit via API</button>
                        <button onClick={() => handleAppSubmit("Portal")} disabled={submittingApp} style={styles.channelButton}>Submit via Portal</button>
                        <button onClick={() => handleAppSubmit("Officer Queue")} disabled={submittingApp} style={styles.channelButton}>Submit via Officer Queue</button>
                      </div>
                      {submissionResult && (
                        <div style={{marginTop: 12}}>
                          <strong>Last submission:</strong> {submissionResult.message || JSON.stringify(submissionResult)}
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div>
                <h3 style={styles.subHeading}>Step 13: Live Application tracking</h3>
                <div style={styles.formGroup}>
                  <label style={styles.formLabel}>Application / Tracking ID</label>
                  <input
                    type="text"
                    value={trackingId}
                ...
                <button onClick={handleTrackStatus} style={styles.actionButton}>Track Application</button>
                {trackingLoading && (<div style={{marginTop:8}}>Checking application status...</div>)}

                {trackingResult && (
                  <div style={styles.stepperContainer}>
                    <h4>Application tracking details:</h4>
                    <div>Scheme: <strong>{trackingResult.scheme_name}</strong></div>
                    <div>Status: <strong style={{color: "#243c8f"}}>{trackingResult.status}</strong></div>
                    <div>Expected Timeline: <strong>{trackingResult.expected_timeline}</strong></div>
                    <div>Officer Remarks: <em style={styles.remarksText}>"{trackingResult.officer_remarks || "No remarks yet."}"</em></div>
                    
                    <h5 style={{margin: "12px 0 6px 0"}}>Workflow Stepper:</h5>
                    <div style={styles.stepperStages}>
                      {trackingResult.stages?.map((stage, idx) => (
                        <div key={idx} style={styles.stepStageItem}>
                          <span style={{
                            ...styles.stepIndicatorDot,
                            backgroundColor: stage.status === "COMPLETED" ? "#176b3a" : "#ccc"
                          }}></span>
                          <div>
                            <strong>{stage.stage}</strong> - <span style={{fontSize: "12px"}}>{stage.status}</span>
                            <p style={styles.subtleText}>{stage.remarks}</p>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        {/* Tab 6: Audit Compliance Logs (Step 14) */}
        {activeTab === "audit" && (
          <div style={styles.panelContent}>
