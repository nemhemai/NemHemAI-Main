...
def run_full_pipeline(job_id, file_path, metadata):
    ...
    try:
        ...
        # On retry (job was failed/stuck), always clear old elements
        # so extraction runs fresh instead of skipping
        # --------------------------------------------------
        with conn.cursor() as cur:
        ...
        if prev_status in ("failed", "FAILED", "extracting", "EXTRACTING"):
            print(f"RETRY detected (prev status={prev_status}) -- clearing old elements for fresh extraction")
            with conn.cursor() as cur:
        ...
        elements, extraction_stats = extractor.extract_elements(file_path)

        print(f"OK: Extraction complete -> {len(elements)} raw elements")
        ...
        for idx, e in enumerate(elements):
            ...
            if not isinstance(e, dict):
                print(f"ERROR: INVALID ELEMENT TYPE at {idx}: {type(e)} -> {e}")
                invalid_count += 1
                continue

            # ensure required keys exist
            if "sequence_order" not in e:
                print(f"ERROR: MISSING sequence_order at {idx}: {e}")
                invalid_count += 1
                continue
