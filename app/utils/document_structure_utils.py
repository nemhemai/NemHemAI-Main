...
def is_toc_page(page_texts):
    ...
    # heuristics for Indian government TOC (relaxed to avoid flagging short forms)
    if numbered_lines >= 15:
        return True

    if short_numeric >= 20:
        return True

    if total > 10 and (numbered_lines + short_numeric) / max(total,1) > 0.60:
        return True
