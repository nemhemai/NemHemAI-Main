...
def preprocess_image_for_ocr(image):
    ...
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # Use GaussianBlur instead of fastNlMeansDenoising to prevent MemoryErrors on large images
...
# -----------------------------
def _ocr_page_worker(input_pdf_path, page_number):
    import fitz
    from PIL import Image
    ...
    applied_preprocessing = should_preprocess(img)

    if applied_preprocessing:
        processed_page = preprocess_image_for_ocr(img)
    else:
        processed_page = img
...
def create_searchable_pdf(input_pdf_path, output_pdf_path=None, debug=True):
    """
